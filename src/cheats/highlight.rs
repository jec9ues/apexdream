...
use crate::*;
use crate::cheats::config::HighlightConfigs;
...
impl Highlight {
    #[inline(never)]
    pub fn run(&mut self, api: &mut Api, ctx: &mut RunContext, config: &HighlightConfigs) {
        ...
        let Some(camera) = state.camera_player() else {
            return;
        };
        ...
        for entity in state.entities() {
            match entity.as_ref() {
                ...
                EntityRef::Loot(loot) if self.config.enable_loot => {
                    self.highlight_prop(api, ctx, loot, camera, &desired_items)
                }
                EntityRef::Deathbox(deathbox) if self.config.enable_deathbox => {
                    self.highlight_deathbox(api, ctx, deathbox, camera)
                }
                EntityRef::Animating(anim) if self.config.enable_animating => {
                    self.highlight_animating(api, ctx, anim, camera)
                }
                _ => (),
            }
        }
    ...
    fn highlight_player(
        ...
        // Choose the style
        let (highlight_id, color) = if player.is_downed() {
            (3, Some([1.0, 1.0, 1.0]))
        ...
        let bits = HighlightBits::new(2, 125, 64, 127, true, false);
    ...
    fn highlight_deathbox(
        &mut self,
        api: &mut Api,
        ctx: &mut RunContext,
        deathbox: &DeathboxEntity,
        camera: &PlayerEntity,
    ) {
    ...
    fn highlight_animating(
        &mut self,
        api: &mut Api,
        ctx: &mut RunContext,
        anim: &AnimatingEntity,
        camera: &PlayerEntity,
    ) {
...
fn highlight(
    ...
    let OFF_GLOW_VISIBLE_TYPE: u32 = 0x26c; //[DT_HighlightSettings].?
    let OFF_GLOW_FIX: u32 = 0x268;
    let OFF_GLOW_HIGHLIGHT_ID: u32 = 0x28c; //[DT_HighlightSettings].m_highlightServerActiveStates
    let OFF_GLOW_HIGHLIGHTS: u32 = 0xb944e30;

    if let Ok(highlight_settings_ptr) =
        api.vm_read::<sdk::Ptr>(ctx.process.base.field(0xbb24350))
    {
        if let Ok(context_id) = api.vm_read::<u8>(entity_ptr.field(0xbb24350)) {
            if let Ok(mut highlight_setting) = api.vm_read::<HighlightSetting>(
                highlight_settings_ptr
                    .field((size_of::<HighlightSetting>() * context_id as usize) as u32),
            ) {
                if context_id >= 0 && ctx.entity_check(api, index, entity_ptr) {
                    if style.enable {
                        // let _ = api.vm_write(entity_ptr.field(0x26c), &2);
                        // let _ = api.vm_write(entity_ptr.field(0x268), &1);
                        // vp.write_raw(highlightSettingsPtr + HIGHLIGHT_TYPE_SIZE * highlight_ID as u64 + 0, [137, 138, 64, 64]);
                        let _ = api.vm_write::<[u8; 4]>(
                            highlight_settings_ptr.field(
                                (size_of::<HighlightSetting>() * context_id as usize) as u32,
                            ),
                            &[137, 138, 64, 64],
                        );
                        // highlight.flag |= sdk::HIGHLIGHT_FLAG_CHECK_OFTEN;
                        // highlight_settings.params.color[0] = color[0] * style.brightness;
                        // highlight_settings.params.color[1] = color[1] * style.brightness;
                        // highlight_settings.params.color[2] = color[2] * style.brightness;
                        // highlight_settings.bits = style.to_bits();
                        // let _ = api.vm_write(highlight_settings_ptr.field((size_of::<HighlightSettings>() * context_id as usize) as u32), &highlight_settings);
                    }
                }
            }
        }
...
fn highlight2(
    ...
    if ctx.entity_check(api, index, entity_ptr) {
        let _ = api.vm_write(entity_ptr.field(0x26c), &2);
        let _ = api.vm_write(entity_ptr.field(0x268), &2);
        let _ = api.vm_write(entity_ptr.field(0x28c), &highlight_id);


        ctx.state.highlight.set(api, highlight_id, bits, color);
    }
    ...
    /*    if let Ok(highlight_settings_ptr) =
            api.vm_read::<sdk::Ptr>(ctx.process.base.field(ctx.data.highlight_settings))
        {
            if let Ok(context_id) = api.vm_read::<u8>(entity_ptr.field(OFF_GLOW_HIGHLIGHT_ID)) {
                if let Ok(mut highlight_setting) = api.vm_read::<HighlightSetting>(
                    highlight_settings_ptr
                        .field((size_of::<HighlightSetting>() * context_id as usize) as u32),
                ) {
                    if context_id >= 0 && ctx.entity_check(api, index, entity_ptr) {
                        if true {
                            ...
                            // vp.write_raw(highlightSettingsPtr + HIGHLIGHT_TYPE_SIZE * highlight_ID as u64 + 0, [2, 125, 64, 127]);
                            // let new_bit = HighlightBits::new(2, 125, 64, 127, true, false);
                            // let _ = api.vm_write::<HighlightBits>(
                            //     highlight_settings_ptr.field(
                            //         (size_of::<HighlightSetting>() * 10 as usize) as u32,
                            //     ),
                            //     &new_bit,
                            // );
                            // let new_color = [color[0] * style.brightness, color[1] * style.brightness, color[2] * style.brightness];
                            // let _ = api.vm_write::<[f32; 3]>(
                            //     highlight_settings_ptr.field(
                            //         (size_of::<HighlightSetting>() * 10 as usize + size_of::<HighlightBits>()) as u32,
                            //     ),
                            //     &new_color,
                            // );
                            // highlight.flag |= sdk::HIGHLIGHT_FLAG_CHECK_OFTEN;
