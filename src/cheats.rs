...
impl<'a> RunContext<'a> {
    ...
    pub fn entity_check(&self, api: &mut Api, index: u32, entity_ptr: sdk::Ptr) -> bool {
        let address = self
            .process
            .base
            .field::<[sdk::CEntInfo]>(self.data.entity_list);
    ...
    pub fn select_slot(&self, api: &mut Api, slot: u8) {
        ...
        let ptr = self
            .process
            .base
            .field(self.data.input + self.data.input_selected_slot);
    ...
    pub fn post(&mut self, api: &mut Api) {
        ...
        fn any_button(this: &RunContext) -> bool {
            if !matches!(this.jump, InState::Default) {
                return true;
            }
            if !matches!(this.duck, InState::Default) {
                return true;
            }
            if !matches!(this.attack, InState::Default) {
                return true;
            }
            if !matches!(this.reload, InState::Default) {
                return true;
            }
            if !matches!(this.inuse, InState::Default) {
                return true;
            }
            if !matches!(this.forward, InState::Default) {
                return true;
            }
            return false;
        }
        ...
        pub fn post(
            ...
            let value = if let InState::Press = state {
                add + 1
            } else if let InState::Release = state {
                add
            } else if button.down[0] == 0 && button.down[1] == 0 {
                add
            } else {
                add + 1
            };
    ...
    pub fn camera_origin(&self) -> [f32; 3] {
        let Some(local) = self.state.local_player() else {
            return [0.0; 3];
        };
        local.camera_origin
    }
